    ...
    while (retries < maxRetries) {
        try {
            ...
            if (retries === maxRetries) {
                console.error(`下载失败 ${url} (重试 ${retries}次后): ${error.message}`);
                return false;
            }
            console.log(`下载失败，正在重试 (${retries}/${maxRetries}): ${url}`);
...
async function scrapeWebsite() {
    try {
            ...
            if (style) {
                ...
                if (matches) {
                    matches.forEach(match => {
                        ...
                        const resource = processResourceUrl(baseUrl, url);
                        if (resource) {
                            downloadResource(resource.absoluteUrl, path.join(__dirname, resource.localPath));
                            $(elem).attr('style', style.replace(url, resource.localPath));
                        }
                    });
                }
            }
        ...
        console.log('网站内容已成功保存到 index.html');
